...
describe('simulate — DC income override', () => {
  it('uses the top-level DC household income for the baseline scenario', () => {
    const dc = getDestination('dc-baseline')!;
    const career = dc.careerPresets[0];
    const higherIncomeGlobals = {
      ...GLOBAL_DEFAULTS,
      currentHouseholdIncome: 300000,
    };

    const result = simulate(dc, career, higherIncomeGlobals, {
      dcHomeDecision: 'keep',
      moveYear: NEXT_YEAR,
      returnYear: null,
    });

    expect(result[0].grossIncome).toBe(300000);
  });
});
...
describe('simulate', () => {
  ...
  it('produces projections for each year from moveYear to retirement', () => {
    ...
    const result = simulate(dest, career, globals, { dcHomeDecision: 'keep', moveYear: NEXT_YEAR, returnYear: null });
  ...
  it('Kenya scenario has lower expenses than DC', () => {
    ...
    const dcResult = simulate(dcDest, dcCareer, globals, { dcHomeDecision: 'keep', moveYear: NEXT_YEAR, returnYear: null });

    const keDest = getDestination('kenya-nairobi')!;
    const keCareer = keDest.careerPresets[0];
    const keResult = simulate(keDest, keCareer, globals, { dcHomeDecision: 'sell', moveYear: NEXT_YEAR, returnYear: null });
  ...
  it('selling DC home adds lump sum to investments', () => {
    ...
    const career = dest.careerPresets[0];
    const sellResult = simulate(dest, career, globals, { dcHomeDecision: 'sell', moveYear: NEXT_YEAR, returnYear: null });
    const keepResult = simulate(dest, career, globals, { dcHomeDecision: 'keep', moveYear: NEXT_YEAR, returnYear: null });
  ...
  it('return-to-DC switches location mid-projection', () => {
    const dest = getDestination('kenya-nairobi')!;
    const career = dest.careerPresets[0];
    const returnYear = NEXT_YEAR + 5;
    const result = simulate(dest, career, globals, { dcHomeDecision: 'rent', moveYear: NEXT_YEAR, returnYear });
    const abroadYears = result.filter((y) => y.location === 'kenya-nairobi');
    const dcYears = result.filter((y) => y.location === 'dc-baseline');
    expect(abroadYears.length).toBe(returnYear - NEXT_YEAR);
